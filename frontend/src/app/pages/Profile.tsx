 ...
 *   GET  /api/auth/profile/      → { id, name, email, phone, date_joined }
 *   PATCH /api/auth/profile/     → { name } → returns same shape
 *   GET  /api/auth/enrollments/  → [{ id, course_title, course_type, duration,
 *                                     status, enrolled_on, preferred_date }]
 *
...
import {
  User, Mail, Phone, Calendar, BookOpen, Award, Clock,
...
interface UserProfile {
  ...
  name: string;
  email: string;
  phone: string;        // backend always returns "" if not set
  date_joined: string;  // backend formats as "Month YYYY" e.g. "January 2025"
}
...
interface Enrollment {
  ...
  enrolled_on: string;       // backend formats as "DD Mon YYYY" e.g. "01 Jan 2025"
...
export function Profile() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/auth/profile/`, { headers: authHeaders() })
      ...
      .catch(err => {
        if (err.message !== 'unauth') setProfileError('Could not load profile.');
      })
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/auth/enrollments/`, { headers: authHeaders() })
      .then(res => {
      ...
      .catch(() => setCoursesError('Could not load your enrollments.'))
  ...
  // ── Save profile (PATCH /api/auth/profile/) ────────────────────────────────
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/auth/profile/`, {
        method: 'PATCH',
        headers: authHeaders(),
        // Backend only accepts 'name' for PATCH
        body: JSON.stringify({ name: form.name.trim() }),
      });
      if (!res.ok) throw new Error('Save failed');
      const updated: UserProfile = await res.json();
      setProfile(updated);
      // Keep localStorage in sync so Navbar reflects the new name immediately
          ...
          <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }}
            ...
            <div className="bg-white dark:bg-[#1c2230] border-2 border-gray-100 dark:border-[#2d3748] rounded-2xl overflow-hidden shadow-sm hover:shadow-lg dark:hover:shadow-[0_8px_30px_rgba(0,0,0,0.4)] transition-shadow duration-300">
              ...
              <div className="px-6 pb-6">
                ...
                <div className="mb-4">
                  <h2 className="text-xl font-extrabold text-gray-900 dark:text-white" style={{ fontFamily: "'Exo 2', sans-serif" }}>
                    {profile?.name}
                  </h2>
                  <p className="text-blue-600 dark:text-blue-400 text-sm font-semibold">SoCloud Student</p>
                </div>
                ...
                <div className="space-y-3 mb-6">
                  {[
                    ...
                    { icon: Calendar, value: profile?.date_joined ? `Joined ${profile.date_joined}` : '—' },
                  ].map((item, i) => {
                    const Icon = item.icon;
                    return (
          ...
          <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }}
              ...
              <div className="space-y-5">
                ...
                {!loadingCourses && !coursesError && enrollments.length === 0 && (
                  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
                    ...
                    <h4 className="font-bold text-gray-900 dark:text-white mb-2 text-lg" style={{ fontFamily: "'Exo 2', sans-serif" }}>
                      No Enrollments Yet
                    </h4>
                  ...
                  <motion.div key={enrollment.id}
                    ...
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-start gap-4">
                        ...
                        <div>
                          <h3 className="font-bold text-gray-900 dark:text-white text-lg mb-1" style={{ fontFamily: "'Exo 2', sans-serif" }}>
                      ...
                      <div className="text-right flex-shrink-0 ml-3">
                        ...
                        {/* Backend returns pre-formatted "DD Mon YYYY" string */}
            ...
            {activeTab === 'security' && (
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
                ...
                <h3 className="text-xl font-extrabold text-gray-900 dark:text-white mb-6" style={{ fontFamily: "'Exo 2', sans-serif" }}>
                  Change Password
                </h3>
                <div className="space-y-5">
                ...
                <div className="mt-8 pt-8 border-t border-gray-100 dark:border-[#2d3748]">
                  <h3 className="text-xl font-extrabold text-gray-900 dark:text-white mb-4" style={{ fontFamily: "'Exo 2', sans-serif" }}>
                    Account Security
                  </h3>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }}
            ...
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-2xl font-extrabold text-gray-900 dark:text-white" style={{ fontFamily: "'Exo 2', sans-serif" }}>
                Edit Profile
              </h3>
            ...
            <div className="space-y-4">
              ...
              {/* Email is read-only — backend does not support changing it */}
              <div>
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={handleSave} disabled={saveLoading || !form.name.trim()}
