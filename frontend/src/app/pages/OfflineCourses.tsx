...
/* ── Static data ─────────────────────────────────────────────────────────────
   The backend's CourseListSerializer returns:
     id, title, slug, course_type, level, duration, price, rating,
     students, topics, location, schedule
   It does NOT return: img, badge, accent, description.
   We keep static data for those display-only fields and merge by title.
──────────────────────────────────────────────────────────────────────────── */
const staticCourses = [
  {
...
/* ── Merge API data with static display fields ───────────────────────────── */
function mergeWithStatic(apiCourses: any[]): Course[] {
  return apiCourses.map((apiCourse, idx) => {
    const match = staticCourses.find(
      s => s.title.toLowerCase().trim() === String(apiCourse.title || '').toLowerCase().trim()
    ) || staticCourses[idx % staticCourses.length];
    return {
      ...match,
      // Override with live API fields (id, title, slug, duration, rating, students, topics, location, schedule)
      id:       apiCourse.id       ?? match.id,
      title:    apiCourse.title    ?? match.title,
      duration: apiCourse.duration || match.duration,
      students: apiCourse.students || match.students,
      rating:   apiCourse.rating   || match.rating,
      location: apiCourse.location || match.location,
      schedule: apiCourse.schedule || match.schedule,
      topics:   Array.isArray(apiCourse.topics) && apiCourse.topics.length > 0
                  ? apiCourse.topics
                  : match.topics,
    };
  });
...
export function OfflineCourses() {
  ...
  useEffect(() => {
    fetch(`${API_URL}/api/courses/offline/`)
      .then(res => {
      ...
      .then((data: any[]) => {
        if (Array.isArray(data) && data.length > 0) {
          // Merge live API data with static display fields
          setCourses(mergeWithStatic(data));
        }
        // If empty array, keep static courses (already the default state)
      })
      .catch(() => {
        // Backend unreachable — static courses already in state, just stop loading
      })
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
              <div>
                ...
                <div className="space-y-4">
                  {[
                    { icon: MapPin,   label: 'Address',  value: 'Plot no.231, 2nd floor, Road no.12, Swamy Ayyappa Society, Madhapur, Hyderabad-500081 \n  Road no.2, Dhanalaxmi Center, ICICI Bank Building, 3rd Floor, KPHB Colony, Hyderabad-500072', sub: 'www.socloudtechnologies.com' },
                    ...
                    <div key={i} className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-gray-500 text-sm whitespace-pre-line">{item.value}</div>
              ...
              <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl h-64 flex items-center justify-center border border-blue-100">
                <div className="text-center">
                  ...
                  <p className="font-bold text-gray-700 text-lg" style={{ fontFamily: "'Exo 2', sans-serif" }}>SoCloud Technologies</p>
