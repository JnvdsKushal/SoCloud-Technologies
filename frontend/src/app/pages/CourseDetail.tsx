...
export function CourseDetail() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      // POST /api/bookings/  → BookingSerializer
      // Backend view reads course_title, mode from request.data for the email
      const res = await fetch(`${API_URL}/api/bookings/`, {
        ...
        body: JSON.stringify({
          ...
          course_title:   course?.title,         // read by view for email
          mode:           formData.mode,          // read by view for email
          message:        formData.message
            ? `Course: ${course?.title} | Mode: ${formData.mode} | ${formData.message}`
            : `Course: ${course?.title} | Mode: ${formData.mode}`,
          course:         null,                   // no FK — free-text enrollment
        }),
      });
      if (!res.ok) {
