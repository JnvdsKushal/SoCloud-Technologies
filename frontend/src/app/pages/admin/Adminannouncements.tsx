...
//
// NOTE: The backend has NO /api/announcements/ endpoint yet.
// This component manages announcements locally in-memory and shows
// them via the AnnouncementBar static ITEMS list.
// To connect to a real backend, add the announcements URLs to
// courses/urls.py and update the fetch calls below.

import { useState } from 'react';
import { Megaphone, Plus, Trash2, ToggleLeft, ToggleRight, GripVertical, Save, X, AlertTriangle } from 'lucide-react';
...
// Default announcements (mirrors AnnouncementBar ITEMS)
const DEFAULT_ITEMS: Announcement[] = [
  { id: 1, text: 'New batch starting for Cyber Security — limited seats available!', badge: 'New Batch', badge_color: 'blue',   link: '', link_label: 'Learn more', is_active: true, order: 1 },
  { id: 2, text: 'Free demo class every Saturday at 10 AM — register now.',         badge: 'Free Demo', badge_color: 'green',  link: '', link_label: 'Learn more', is_active: true, order: 2 },
  { id: 3, text: 'AWS & Azure certifications — early bird discount ends soon.',      badge: 'Offer',     badge_color: 'orange', link: '', link_label: 'Learn more', is_active: true, order: 3 },
];

export function AdminAnnouncements() {
  const [items, setItems]       = useState<Announcement[]>(DEFAULT_ITEMS);
  ...
  const openEdit   = (item: Announcement) => { const { id, ...rest } = item; setForm(rest); setEditId(id); setShowForm(true); };
    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="mb-6 flex items-start gap-3 px-4 py-3 rounded-xl bg-amber-50 border border-amber-200">
        ...
        <div className="text-sm text-amber-800">
          <strong>No backend endpoint yet.</strong> Changes here are in-memory only and won't persist across page refreshes.
          To persist announcements, add <code className="bg-amber-100 px-1 rounded">/api/announcements/</code> routes to your Django <code className="bg-amber-100 px-1 rounded">urls.py</code>.
        </div>
      </div>
