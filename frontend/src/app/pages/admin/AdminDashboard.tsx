 ...
 * Backend endpoint:
 *   GET /api/admin/stats/ → {
 *     total_courses, active_courses, online_courses, offline_courses,
 *     total_bookings, pending_bookings, confirmed_bookings, cancelled_bookings,
 *     total_students,
 *     recent_bookings: BookingSerializer[]  ← last 5, ordered -created_at
 *   }
 *
 * BookingSerializer fields (for recent_bookings):
 *   id, course (FK id|null), course_title (read_only), name, email, phone,
...
interface Booking {
  id: number;
  name: string;
  course_title: string;   // may be "" if no course FK
                    ...
                    {stats.recent_bookings.map((b, i) => (
                      ...
                      >
                        ...
                        <div className="shrink-0 text-right ml-3">
                          <span className={`inline-flex items-center gap-1 text-[10px] font-bold uppercase px-2 py-0.5 rounded-full ${STATUS_STYLE[b.status] || STATUS_STYLE.pending}`}>
                            {STATUS_ICON[b.status] || STATUS_ICON.pending} {b.status}
                          </span>
