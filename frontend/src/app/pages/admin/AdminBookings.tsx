 ...
 *
 * Backend endpoints:
 *   GET   /api/admin/bookings/        → BookingSerializer[] (all bookings, ordered -created_at)
 *   PATCH /api/admin/bookings/{id}/   → { status } → updated BookingSerializer
 *
 * BookingSerializer fields:
 *   id, course (FK id|null), course_title (read_only via source='course.title'),
 *   name, email, phone, booking_type, preferred_date, message, status, created_at
 */
...
interface Booking {
  ...
  phone: string;
  course: number | null;
  course_title: string;   // read_only, from course FK; may be empty string if no FK
  booking_type: string;   // 'demo' | 'consultation' | 'enrollment'
...
export function AdminBookings() {
  ...
  const filtered = bookings.filter(b => {
    ...
    const matchSearch = [b.name, b.email, b.course_title].some(
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-3 min-w-0 flex-1">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-500">
                      ...
                      {b.preferred_date && (
                        <span className="flex items-center gap-1"><Calendar size={11} />{b.preferred_date}</span>
                      )}
                ...
                <div className="shrink-0 flex flex-col items-end gap-2">
                  <select value={b.status} onChange={e => updateStatus(b.id, e.target.value)}
                    className={`px-3 py-1.5 rounded-full text-xs font-bold border-2 outline-none cursor-pointer transition-all ${BADGE[b.status] || BADGE.pending}`}
                  >
