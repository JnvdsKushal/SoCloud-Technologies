 ...
 *
 * Backend endpoints:
 *   GET    /api/admin/courses/            → CourseSerializer[] (all courses)
 *   POST   /api/admin/courses/            → CourseSerializer (create)
 *   GET    /api/admin/courses/{slug}/     → CourseSerializer (retrieve)
 *   PUT    /api/admin/courses/{slug}/     → CourseSerializer (full update)
 *   PATCH  /api/admin/courses/{slug}/     → CourseSerializer (partial update)
 *   DELETE /api/admin/courses/{slug}/     → 204
 *
 * CourseSerializer uses fields = '__all__', so it includes every model field.
 * Key fields: id, title, slug, description, course_type, level, duration,
 *             price, rating, students, topics (JSONField), location,
 *             schedule, is_active
 */
...
export function AdminCourses() {
  ...
  const openEdit = (c: Course) => {
    setForm({
      ...
      course_type: c.course_type, level: c.level, duration: c.duration,
      price: c.price, rating: c.rating, students: c.students,
      topics: c.topics, location: c.location, schedule: c.schedule,
  ...
  const closeForm = () => { setShowForm(false); setEditSlug(null); setError(''); };
  ...
  const handleSave = async () => {
    ...
    const url    = editSlug
      ? `${API_URL}/api/admin/courses/${editSlug}/`
  ...
  const handleDelete = async (c: Course) => {
    ...
    setDeleting(c.id);
    try {
      const res = await fetch(`${API_URL}/api/admin/courses/${c.slug}/`, { method: 'DELETE' });
  ...
  const handleToggleActive = async (c: Course) => {
    try {
      const res = await fetch(`${API_URL}/api/admin/courses/${c.slug}/`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  const F = <K extends keyof typeof form>(name: K, value: typeof form[K]) =>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    ...
                    {c.level && (
                      <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-slate-100 text-slate-600">{c.level}</span>
                    )}
                  </div>
                  <p className="text-sm text-slate-500 mb-3 line-clamp-2">{c.description}</p>
                  <div className="flex flex-wrap gap-4 text-xs text-slate-500">
                    {c.duration && <span className="flex items-center gap-1"><Clock size={12} className="text-blue-500" />{c.duration}</span>}
                    {c.students && <span className="flex items-center gap-1"><Users size={12} className="text-blue-500" />{c.students}</span>}
                    {c.price    && <span className="flex items-center gap-1"><DollarSign size={12} className="text-blue-500" />{c.price}</span>}
                  ...
                  {Array.isArray(c.topics) && c.topics.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mt-3">
                      ...
                      {c.topics.length > 6 && (
                        <span className="text-[10px] px-2 py-0.5 bg-slate-100 text-slate-400 rounded-full">+{c.topics.length - 6}</span>
                      )}
                    </div>
                  )}
                </div>
          ...
          <>
            ...
            >
              ...
              <div className="overflow-y-auto max-h-[75vh] px-6 py-5">
                {error && (
                  <div className="mb-4 px-3 py-2.5 rounded-xl bg-rose-50 border border-rose-200 text-rose-700 text-xs">{error}</div>
                )}
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">
                      Slug <span className="normal-case font-normal">(auto-generated if blank)</span>
                    </label>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <select value={form.course_type} onChange={e => F('course_type', e.target.value as 'online' | 'offline')}
                        className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-sm outline-none focus:border-blue-400 transition-all"
                      >
