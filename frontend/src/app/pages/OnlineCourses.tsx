...
/* ── Static data ─────────────────────────────────────────────────────────────
   ...
   The slug from the backend is used for the course detail link.
──────────────────────────────────────────────────────────────────────────── */
const staticCourses = [
  {
...
function mergeWithStatic(apiCourses: any[]): Course[] {
  return apiCourses.map((apiCourse, idx) => {
    ...
    return {
      ...match,
      // Override with live API fields
      id:       apiCourse.id       ?? match.id,
      title:    apiCourse.title    ?? match.title,
      // Use backend slug if available (important for course detail links)
      slug:     apiCourse.slug     || match.slug,
      level:    apiCourse.level    || match.level,
...
export function OnlineCourses() {
  ...
  useEffect(() => {
    fetch(`${API_URL}/api/courses/online/`)
      .then(res => {
      ...
      .then((data: any[]) => {
        if (Array.isArray(data) && data.length > 0) {
          // Merge live API data (id, slug, title, duration, rating, etc.)
          // with static display fields (img, badge, accent, description)
