...
export function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // POST /api/auth/register/  → { message, user: { id, name, email } }
      ...
      >
        ...
        >
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-sm" style={{ color:"rgba(255,255,255,0.45)" }}>Join SoCloud and start your learning journey</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <motion.button type="submit" whileHover={{ scale:1.015, y:-1 }} whileTap={{ scale:0.985 }}
              ...
              style={{ background: "#ffffff", color: "#000", border: "1px solid rgba(0,0,0,0.1)" }}
              onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = "#1c1c1c"; (e.currentTarget as HTMLElement).style.color = "#fff"; }}
              onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = "#ffffff"; (e.currentTarget as HTMLElement).style.color = "#000"; }}>
              Create Account
            </motion.button>
          </form>
          ...
          <div className="mt-4 text-center text-[11px] space-y-0.5" style={{ color:"rgba(255,255,255,0.2)" }}>
            ...
            <p>Expected response: {"{ \"message\": \"...\", \"user\": {...} }"}</p>
          </div>
        </motion.div>
      </div>
    </div>
