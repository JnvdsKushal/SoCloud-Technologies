...
export function Booking() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Backend view also reads course_title, mode, preferred_time from request.data
      const res = await fetch(`${API_URL}/api/bookings/`, {
        ...
        body: JSON.stringify({
          ...
          course_title:   formData.course,      // read by view for email
          booking_type:   'demo',
          preferred_date: formData.date || null,
          preferred_time: formData.time || null, // read by view for email
          message:        formData.message,
          mode:           formData.mode,         // read by view for email
          course:         null,                  // no FK — free-text booking
        }),
      });
